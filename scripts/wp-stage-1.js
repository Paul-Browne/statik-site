...
function writeOutAll(data){
	mkdirp(contentDirectoryPath + "/data/wp-json/" + data[0] + "/", function(err) {
        if (err) {
            console.error(err);
        } else {
        	if(data[2]){
        		fs.writeFile(contentDirectoryPath + "/data/wp-json/" + data[0] + ".slug.json", JSON.stringify(data[2]), function(err) {
        		    if (err) {
        		        console.error(err);
        		    }
        		});
        		...
        		fs.writeFile(contentDirectoryPath + "/data/wp-json/" + data[0] + ".id.json", JSON.stringify(data[1]), function(err) {
        		    if (err) {
        		        console.error(err);
        		    }
        		});
